...
void copyFGFixCorner2(void) {
    ...
    // file name for writing (negative value)
    char fnamew[] = "prm/sprm_corner0.5neg_000.bin";
...
Sprm makeChildAverageSprmFC2(Sprm mother, Sprm father) {
    Sprm child;
    // the corner
...
Sprm makeChildCrossMSprmFC2(Sprm mother, Sprm father) {
    Sprm child;
    // the corner
    child.weight[0] = CORNER_EVAL;
    // the others
...
int nextGenerationSprmFC2(int gene_num) {
    ...
    for (i = 0; i < SURVIVE_NUM - 1; i++) {
        for (j = i + 1; j < SURVIVE_NUM; j++) {
            // average
            generation[count] = makeChildAverageSprmFC2(parents[i], parents[j]);
            count++;
            // cross (not mutate)
            generation[count] = makeChildCrossMSprmFC2(parents[i], parents[j]);
            count++;
        }
    }
    // calcurate survivors
...
// loop several times
void nextGenerationSprmFC2L(int st, int loop) {
    time_t t0, t1;
    // get start time
    time(&t0);
    for (int i = st; i < st + loop; i++) {
        nextGenerationSprmFC2(i);
        // get time
        time(&t1);
