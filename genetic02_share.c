...
int makeSprmFileNameRankGeoProgPy(char *dst, int dst_size, int loc_pop, int loc_eln, int sel_id, int crs_id, int loc_seed, double cmn_ratio, int gene_num, int options) {
    ...
    if (makeSprmFileFormatAuto(tmp_str, FILENAME_MAX, 0b10 | norm, 1, loc_pop, loc_eln, sel_id, crs_id, 0, .01, loc_seed, cmn_ratio, ALPHA_BLX, zako) < 0) {
        return -1;
    }
    int l;
    // 世代番号を指定
...
int getBestActABSprmPy(int b_arr[MASU_NUM], int depth_max, int turn) {
    ...
    if ((n = canPutPP(b, turn, cpa, nba, kc))) {
        for (int i = 0; i < n; i++) {
            // 既に 1 手先の盤面は見ているため, 深さは1減らして渡す
            if ((tp = negaMaxABSprmPy(nba[i], turn ^ 0b11, depth_max - 1, 0, -FLT_MAX, -alpha)) > alpha) {
                alpha = tp;
                best_te = cpa[i];
            }
        }
