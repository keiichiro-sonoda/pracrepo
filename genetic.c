...
int vsTopGeneration(int gene_num) {
    ...
    int pass = 0;
    int turn = 0b01;    // black
    Board main_board = START; // initial board
    Param topg;
    FILE *fp;
    Family gene;
    snprintf(fnamer, FILENAME_MAX, format, gene_num);
    ...
    if ((fp = fopen(fnamer, "rb")) == NULL) {
        ...
        fread(&gene, sizeof gene, 1, fp);
        fclose(fp);
    }
    topg = gene.prms[0];    // get top
    while (1) {
        printf("turn: %c\n", B2C[turn]);
        nc = canPutPP(main_board, turn, cpa, nba, kc);
        showCanPut(main_board, cpa, nc);
        // cannot put a piece anywhere
        if (nc == 0) {
            // cannot do anything each other
            if (pass) {
        ...
        if (turn == 0b01) {
            //main_board = getBestBoard(nba, nc, 0b01, sente);
            act = getValidActStdin(cpa, nc);
            i = getIndex(cpa, nc, act);
            main_board = nba[i];
        } // white
        else {
            main_board = getBestBoard(nba, nc, 0b10, topg);
        }
        // next turn;
        turn ^= 0b11;
    }
